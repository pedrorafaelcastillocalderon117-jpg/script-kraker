#!/bin/bash 
 ...
 install_python(){ 
  ...
  sleep 3  
  screen -dmS pydic-80 python ${SCPinst}/python.py 80 "Kraker-VPS" && echo "80 Kraker-VPS" >> /etc/Kraker-VPS/PySSL.log
  msg -bar
  } 
 ...
 rm /etc/Kraker-VPS/PySSL.log &>/dev/null
 rm -rf /etc/stunnel/certificado.zip private.key certificate.crt ca_bundle.crt &>/dev/null
 clear
 msg -bar
 ...
 for port in $(cat /etc/Kraker-VPS/PortPD.log| grep -v "nobody" |cut -d' ' -f1)
 do
 PIDVRF3="$(ps aux|grep pydic-"$port" |grep -v grep|awk '{print $2}')"
 if [[ -z $PIDVRF3 ]]; then
 screen -dmS pydic-"$port" python /etc/Kraker-VPS/protocolos/python.py "$port"
 else
