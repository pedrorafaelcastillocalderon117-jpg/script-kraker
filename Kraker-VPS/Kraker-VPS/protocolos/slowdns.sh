#!/bin/bash
 #by @rufu99
 ADM_inst="/etc/Kraker-VPS/Slow/install" && [[ ! -d ${ADM_inst} ]] && exit
 ADM_slow="/etc/Kraker-VPS/Slow/Key" && [[ ! -d ${ADM_slow} ]] && exit
 info(){
 	clear
 	nodata(){
 		msg -bar
 ...
 ini_slow(){
     ...
     if [[ ! -e ${ADM_inst}/dns-server ]]; then
     	...
     	if wget -O ${ADM_inst}/dns-server https://raw.githubusercontent.com/NetVPS/Kraker-VPS_Oficial/master/LINKS-LIBRERIAS/dns-server &>/dev/null ; then
