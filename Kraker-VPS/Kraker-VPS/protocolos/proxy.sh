#!/bin/bash
 ...
 SCPinst="/etc/Kraker-VPS/protocolos"
 mportas () {
 unset portas
 ...
 det_py(){
 	...
 	rm /etc/Kraker-VPS/PortPD.log &>/dev/null
 	rm -rf /etc/Kraker-VPS/protocolos/lacasitamx.py
 ...
 tput cuu1 && tput dl1
     ...
     echo -e "$BARRA"
     [[ ! -e /etc/Kraker-VPS/protocolos/lacasitamx.py ]] && rm -rf /etc/Kraker-VPS/protocolos/lacasitamx.py
 #
 	# Inicializando o Proxy
 	(
 	less << CPM > /etc/Kraker-VPS/protocolos/lacasitamx.py
 # -*- coding: utf-8 -*-
...
CPM
 ...
 chmod +x /etc/Kraker-VPS/protocolos/lacasitamx.py &>/dev/nulll
 
 screen -dmS pydic-"$proxycolor" python ${SCPinst}/lacasitamx.py "$proxycolor" "$mensage" && echo ""$proxycolor" "$mensage"" >> /etc/Kraker-VPS/PortPD.log
 }
