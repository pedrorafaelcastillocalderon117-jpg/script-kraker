...
View_User_info() {
	...
	msg -bar
	echo -e " PANEL Kraker-VPS By @Kalix1"
...
Set_config_user() {
	...
	msg -bar
	stty erase '^H' && read -p "(Predeterminado: Kraker-VPS):" ssr_user
	[[ -z "${ssr_user}" ]] && ssr_user="Kraker-VPS"
...
Set_config_password() {
	...
	msg -bar
	stty erase '^H' && read -p "(Predeterminado: Kraker-VPS):" ssr_password
	[[ -z "${ssr_password}" ]] && ssr_password="Kraker-VPS"
